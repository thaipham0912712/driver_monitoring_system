    ...
    for ID, folder in enumerate(folder_list):
        ...
        for txt_file in files_per_person:
            if txt_file == 'alert.txt':
                ...
                alert_blink_unrolled = unroll_in_time(
                    normalized_blinks, window_size, stride)
            ...
            if txt_file == 'semisleepy.txt':
                ...
                semi_blink_unrolled = unroll_in_time(
                    normalized_blinks, window_size, stride)
            ...
            if txt_file == 'sleepy.txt':
                ...
                sleepy_blink_unrolled = unroll_in_time(
                    normalized_blinks, window_size, stride)
                sleepy_labels = 10 * np.ones([len(sleepy_blink_unrolled), 1])

        tempX = np.concatenate(
            (alert_blink_unrolled, semi_blink_unrolled, sleepy_blink_unrolled), axis=0)
        tempY = np.concatenate(
            (alert_labels, semi_labels, sleepy_labels), axis=0)
        if ID > 0:
...
def Preprocess(path1, window_size, stride, test_fold):
    # param: path1 is the address to the folder of all subjects, each subject has three txt files for alert, semisleepy and sleepy levels
    # param: window_size decides the length of blink sequence
    # param: stride is the step by which the moving window slides over consecutive blinks to generate the sequences
    # param: test_fold is the number of fold that is picked as test and uses the other folds as training
    # return: output=[N,T,F]
    path = path1
