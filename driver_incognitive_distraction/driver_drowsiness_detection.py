...
def emergency(ear, COUNTER):
    if ear < EAR_THRESH:
        COUNTER += 1
        if COUNTER >= 50:
            logging.warning('EMERGENCY SITUATION (EYES TOO LONG CLOSED)')
...
def blink_detector(input_video=None):
    logging.info("Start blink detecting")
    output_textfile = "detected_blink.txt"
    ...
    def Ultimate_Blink_Check():
        ...
        if numberOfblinks>1:
            MISSED_BLINKS=True
        if numberOfblinks ==0:
            logging.info(f"{Updown}, {Last_Blink.duration}")
            logging.info(values)
            logging.info(Derivative)
        ...
        # otherwise, the eyes are open in this frame
                ...
                if ( (reference_frame-6) - Last_Blink.end) > FRAME_MARGIN_BTW_2BLINKS:
                    ...
                    if  Last_Blink.peakEAR < Last_Blink.startEAR and Last_Blink.peakEAR < Last_Blink.endEAR and Last_Blink.amplitude>MIN_AMPLITUDE and Last_Blink.start<Last_Blink.peak:
                        if((Last_Blink.startEAR - Last_Blink.peakEAR)> (Last_Blink.endEAR - Last_Blink.peakEAR)*0.25 and (Last_Blink.startEAR - Last_Blink.peakEAR)*0.25< (Last_Blink.endEAR - Last_Blink.peakEAR)): # the amplitude is balanced
                            ...
                            Counter4blinks = 0
                            logging.info("MISSED BLINKS= {}".format(len(retrieved_blinks)))
                            return retrieved_blinks,int(TOTAL_BLINKS),Counter4blinks,BLINK_READY,skip
                        else:
                            skip=True
                            logging.info('rejected due to imbalance')
                    else:
                        skip = True
                        logging.info('rejected due to noise,magnitude is {}'.format(Last_Blink.amplitude))
                        logging.info(Last_Blink.start<Last_Blink.peak)
            ...
            if Counter4blinks >1:
                ...
                if (Current_Blink.start-Last_Blink.end )<=FRAME_MARGIN_BTW_2BLINKS+1:  #Merging two close blinks
                    ...
                    logging.info(f"{Current_Blink.start} ,{Last_Blink.end}, {frames_in_between}")
    ...
    logging.info(f"Start Blink detector for {input_video}")
    ...
    if input_video is None:
        logging.info("Start video streamming")
        stream = cv2.VideoCapture(0)
    else:
        logging.info("Scanning input video")
        stream = cv2.VideoCapture(input_video)
    # start = datetime.datetime.now()
    number_of_frames=0
    while True:
        (grabbed, frame) = stream.read()
        # Check for breaking the loop at the end of the video
        if not grabbed:
            logging.info('End video')
            logging.info(number_of_frames)
            break
        ...
        # we only consider frames that face is detected
        if (np.size(rects) != 0):
            ...
            if Q.full() and (reference_frame>15):  #to make sure the frame of interest for the EAR vector is int the mid
                ...
                if (BLINK_READY==True):
                    ...
                    for detected_blink in retrieved_blinks:
                        logging.info(f"{detected_blink.amplitude}, {Last_Blink.amplitude}")
                        logging.info(f"{detected_blink.duration}, {detected_blink.velocity}")
                        logging.info('-------------------')

                        if(detected_blink.velocity>0):
                            with open(output_textfile, 'ab') as f_handle:
                                f_handle.write(b'\n')
        ...
        #Does not detect any face
            ...
            if Q.full() and (reference_frame>15):
                ...
                if (BLINK_READY==True):
                    ...
                    for detected_blink in retrieved_blinks:
                        logging.info(f"{detected_blink.amplitude}, {Last_Blink.amplitude}")
                        logging.info(f"{detected_blink.duration}, {Last_Blink.duration}")
                        logging.info('-------------------')
                        with open(output_textfile, 'ab') as f_handle:
                            f_handle.write(b'\n')
                            np.savetxt(f_handle,[TOTAL_BLINKS,BLINK_FRAME_FREQ*100,detected_blink.amplitude,detected_blink.duration,detected_blink.velocity], delimiter=', ', newline=' ',fmt='%.4f')
    ...
    logging.info(f"Finish!!! Output file have been parsered in {output_textfile}")

def arg_parse():
    parser = argparse.ArgumentParser(description='Blinks detector')
