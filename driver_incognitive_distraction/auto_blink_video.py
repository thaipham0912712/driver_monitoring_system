  ...
  for root, dirs, files in os.walk(path):
    for file in files:
      if(file.endswith(".mp4") or file.endswith(".mov") or file.endswith(".MOV")):
        ...
        # process = subprocess.run(["python", "blink_video.py", "-o", os.path.join(root,output_name) ,"-i", os.path.join(root,file)],shell=True, cwd=None, check=True, capture_output=True)
