...
def regression_per_blink(labels, predictions, starts_list,
    ...
    for i, start in enumerate(starts_list):
        ...
        if Y[0, 0] == 5:
            loss = np.sum((predicts[np.logical_and(
                predicted_labels != Y[0, 0], predicted_labels < 5)] - 3.3) ** 2)
            loss = loss + \
                np.sum((predicts[np.logical_and(
                    predicted_labels != Y[0, 0], predicted_labels >= 5)] - 6.6) ** 2)
        else:
            loss = np.sum((predicts[predicted_labels != Y[0, 0]] - L) ** 2)
        LOSS = LOSS + loss

    if idx % 15 == 0 or idx == 79:
        print('Per Blink Sequence Regression Error is :%f ' %
              (LOSS / len(labels)))
...
def vote_accuracy_per_video(labels, predictions, starts_list, idx):
    ...
    for i, start in enumerate(starts_list):
        ...
        lowVigilant_voted_percent = np.sum(
            predicted_labels == labels_pool[1]) / len(Y)
        drowsy_voted_percent = np.sum(
            predicted_labels == labels_pool[2]) / len(Y)
        max = np.max(
            [alert_voted_percent, lowVigilant_voted_percent, drowsy_voted_percent])
        ...
        if np.sum([alert_voted_percent, lowVigilant_voted_percent, drowsy_voted_percent] == max) != 1:
            ...
            final_decision = labels_pool[np.argmax(
                [alert_voted_percent, lowVigilant_voted_percent, drowsy_voted_percent])]
        if final_decision == Y[0, 0]:
            count = count+1
        if idx % 10 == 0:
            print(str(
                i+1)+': '+'True label is :%d and the detected label is %d' % (Y[0, 0], final_decision))
...
def batchNorm(x, beta, gamma, training, scope='bn'):
    with tf.variable_scope(scope):
        ...
        mean, var = tf.cond(training, mean_var_with_update, lambda: (
...
def Network(input, Pre_fc1_size, Post_fc1_size_per_layer, embb_size, embb_size2, Post_fc2_size, hstate_size, num_layers, feature_size,
    ...
    with tf.variable_scope('pre_fc1'):
        ...
        input_RNN = batchNorm(input_RNN, pre_fc1_biases,
                              None, training, scope='bn')
        input_RNN = tf.nn.relu(input_RNN)
        # size=[batch,Time,Pre_fc1_size ]
        input_RNN = tf.reshape(input_RNN, [-1, step_size, Pre_fc1_size])
    ...
    with tf.variable_scope('post_fc1'):
        ...
        for lay in range(num_layers):
            ...
            post_fc1_biases = tf.get_variable('biases_%s' % lay, [Post_fc1_size_per_layer], dtype=tf.float32,
                                              initializer=tf.constant_initializer(0.0))
            # size of output_RNN[lay] is (step,batch,hsize),
            trash, output_RNN = tf.split(
                output_RNN_set[lay], [step_size-1, 1], axis=0, name='layers')
            ...
            post_fc1 = batchNorm(post_fc1, post_fc1_biases,
                                 None, training, scope='bn')

            if lay == 0:
                post_fc1_out = post_fc1
            else:
                # size=[Batch,layer*Post_fc1_size_per_layer]
                post_fc1_out = tf.concat([post_fc1_out, post_fc1], axis=1)
    ...
    with tf.variable_scope('embeddings'):
        ...
        embeddings = batchNorm(
            embeddings, embeddings_biases, None, training, scope='bn')
    ...
    with tf.variable_scope('embeddings2'):
        ...
        embeddings2 = batchNorm(
            embeddings2, embeddings_biases2, None, training, scope='bn')
    ...
    with tf.variable_scope('post_fc2'):
        ...
        post_fc2_out = batchNorm(
            post_fc2_out, post_fc2_biases, None, training, scope='bn')
...
# data=[Total data points, T,F]   #label=[Total Data Points, 1]
def batch_gen(data, label, batch_size):
    n = len(data)
...
# data=[Total data points, T,F]  # This generates epochs of batches of data
def epoch_gen(data, label, batch_size, num_epochs):
...
def train(total_input, total_labels, TestB, TestL, output_size, feature_size, batch_size, num_epochs, Pre_fc1_size, Post_fc1_size_per_layer, embb_size,
    ...
    L2loss = 0
    input_net = tf.placeholder(tf.float32, shape=(
        None, None, feature_size), name='bacth_in')
    labels = tf.placeholder(tf.float32, shape=(
        None, output_size), name='labels_net')  # size=[batch,1]
    ...
    with tf.Session() as sess:
        ...
        ####
        # set of batches in one epoch
        for idx, epoch in enumerate(epoch_gen(data=total_input, label=total_labels, batch_size=batch_size, num_epochs=num_epochs)):
            loss_per_epoch = 0
            sum = 0
            if load == False:
                for b, (X, Y) in enumerate(epoch):  # each batch in each epoch
                    ...
                    # calculating the moving average for acc
                    sum = sum+accuracy
            ...
            # Testing
            if output_size == 1:
                loss_values_Test, predicts_Test, mid_vT = sess.run([loss, output, end_points], feed_dict={
                                                                   input_net: TestB, labels: TestL, keep_p: 1.0, training: False})
                accuracy_Test = calc_accuracy_per_batch(
                    TestL, predicts_Test)  # BSA
                accuracy_per_videoV = vote_accuracy_per_video(
                    TestL, predicts_Test, start_i, idx)  # VA
            ###########
            if load == False:
                print("For Training: "+str(loss_per_epoch) +
                      " , "+str(moving_accuracy))
...
if __name__ == "__main__":
    load = True  # Load the weights from disk if True
    for i in range(5):  # Cross validation but recommended to run each fold a few times to see the best perfomrance as you may
        ...
        BlinksTest[:, :, 0] = (
            BlinksTest[:, :, 0]-np.mean(Blinks[:, :, 0]))/np.std(Blinks[:, :, 0])
        Blinks[:, :, 0] = (
            Blinks[:, :, 0]-np.mean(Blinks[:, :, 0]))/np.std(Blinks[:, :, 0])
        #####
        BlinksTest[:, :, 1] = (
            BlinksTest[:, :, 1]-np.mean(Blinks[:, :, 1]))/np.std(Blinks[:, :, 1])
        Blinks[:, :, 1] = (
            Blinks[:, :, 1]-np.mean(Blinks[:, :, 1]))/np.std(Blinks[:, :, 1])
        #####
        BlinksTest[:, :, 2] = (
            BlinksTest[:, :, 2]-np.mean(Blinks[:, :, 2]))/np.std(Blinks[:, :, 2])
        Blinks[:, :, 2] = (
            Blinks[:, :, 2]-np.mean(Blinks[:, :, 2]))/np.std(Blinks[:, :, 2])
        #####
        BlinksTest[:, :, 3] = (
            BlinksTest[:, :, 3]-np.mean(Blinks[:, :, 3]))/np.std(Blinks[:, :, 3])
        Blinks[:, :, 3] = (
            Blinks[:, :, 3]-np.mean(Blinks[:, :, 3]))/np.std(Blinks[:, :, 3])
        ####
        ...
        if load == False:
            ...
            np.save(open('./accuracy%d.npy' %
                    ii, 'wb'), accuracy)  # for training
            np.save(open('./loss%dTest.npy' % ii, 'wb'), loss_Test)  # for test
            np.save(open('./accuracy%dTest.npy' % ii, 'wb'),
                    accuracy_Test)  # for test (BSA)
            np.save(open('./accuracy%dVTest.npy' % ii, 'wb'),
                    acc_per_Vid)  # for test    (VA)
